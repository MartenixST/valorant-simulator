...
export function displayMatchDetails() {
    ...
    if (team1ActivePlayers.length < 5 && team1Roster.length > 0) {
        ...
        // Debugging specific players for troubleshooting
        const ethan = team1Roster.find(p => p.name.toLowerCase().includes('ethan'));
        const adverso = team1Roster.find(p => p.name.toLowerCase().includes('adverso'));
        if (ethan) console.log(`[DEBUG] Ethan found in roster. Status: ${ethan.status}, Starter: ${ethan.isStarter}`);
        if (adverso) console.log(`[DEBUG] Adverso found in roster. Status: ${adverso.status}, Starter: ${adverso.isStarter}`);

        // Filter available players
        let available = team1Roster.filter(p => 
            !team1ActivePlayers.some(ap => ap.id === p.id) && isActive(p)
        );
        
        // Sort: Starters first, then high overall
        available.sort((a, b) => {
            const aStarter = (a.isStarter === true || a.isStarter === 'true');
            const bStarter = (b.isStarter === true || b.isStarter === 'true');
        ...
        console.log(`Auto-selected ${toAdd.length} active players for Team 1. Total: ${team1ActivePlayers.length}`);
    }
    
    if (team2ActivePlayers.length < 5 && team2Roster.length > 0) {
        const needed = 5 - team2ActivePlayers.length;
        let available = team2Roster.filter(p => 
            !team2ActivePlayers.some(ap => ap.id === p.id) && isActive(p)
        );
        ...
        console.log(`Auto-selected ${toAdd.length} active players for Team 2. Total: ${team2ActivePlayers.length}`);
    }

    if (team1ActivePlayers.length > 0 || team2ActivePlayers.length > 0) {
        // VALIDATION: Ensure all loaded players are still 'active' in the current roster
        // This handles the case where a user benches a player AFTER a match state was saved.
        const validatePlayers = (activePlayers, fullRoster) => {
            return activePlayers.filter(p => {
                const rosterPlayer = fullRoster.find(rp => rp.id === p.id);
                if (!rosterPlayer) return false; // Player no longer on team
                if (!isActive(rosterPlayer)) {
                    console.log(`Removing inactive player ${p.name} (Status: ${rosterPlayer.status}) from active slots.`);
                    return false;
                }
                return true;
            });
        ...
        team1ActivePlayers = validatePlayers(team1ActivePlayers, team1Roster);
        team2ActivePlayers = validatePlayers(team2ActivePlayers, team2Roster);
...
function saveMatchResult(winnerTeam) {
    ...
    if (activeSaveId && activeSave) {
        ...
        // Handle Regular Season Playoffs
        if (matchId && matchId.startsWith('RS-') && activeSave.regularSeason) {
             const rsState = activeSave.regularSeason;
             if (rsState.playoffs) {
                 ...
                 regions.forEach(region => {
                     ...
                     if (!rsState.playoffs[region]) return;
                     ...
                     if (match) {
                         ...
                         // Determine winnerId
                         const winnerObj = teams.find(t => t.name === winnerName);
                         ...
                         // Add to match history if not already there
                         if (!rsState.matches) rsState.matches = [];
                         if (!rsState.matches.find(m => m.id === match.id)) {
                             // Determine week based on match ID patterns or explicit logic
                             let week = 0;
                             if (match.id.includes('QF')) week = 17;
                             else if (match.id.includes('UB-SF') || match.id.includes('LB-R1')) week = 18;
                             else if (match.id.includes('UB-F') || match.id.includes('LB-R2')) week = 19;
                             else if (match.id.includes('LB-R3')) week = 20;
                             else if (match.id.includes('LB-F')) week = 21;
                             else if (match.id.includes('GF')) week = 22;
                             
                             rsState.matches.push({
                                week: week,
                                id: match.id,
                                t1Name: match.team1,
                                t2Name: match.team2,
                         ...
                         matchFound = true;
                         console.log(`Updated Regular Season Playoffs state for match ${matchId}`);
                     }
                 });
             }
        ...
        // Handle Kickoff
        if (matchId && !matchId.startsWith('M-') && !matchId.startsWith('RS-')) {
    ...
    team2ActivePlayersElement = document.getElementById('team2-active-players');
     ...
     if (returnToBracketButton) {
        returnToBracketButton.addEventListener('click', () => {
            ...
            } else if (tournament === 'masters_tokyo') {
