...
export function renderKickoff(st) {
  try {
    ...
    const activeSaveId = localStorage.getItem('activeSaveId');
    const playerTeam = activeSave ? activeSave.team : 'TBD';
    const region = st.region || (activeSave ? activeSave.region : "Americas");
    ...
    if (!st || st.dirty || !st.playInRound1 || st.playInRound1.length === 0 || !st.ubRound1 || st.ubRound1.length === 0 || !st.lbRound4 || st.lbRound4.length === 0) {
      console.log(`Initializing new bracket for ${region} with byes...`);
      let initialUsedTeams = []; // Start with an empty array of used teams

      // Select 4 teams for byes (placeholder for champion teams)
      const byeTeams = [];
      for (let i = 0; i < 4; i++) {
          let team = getRandomTeam(initialUsedTeams, region); // Get a unique team
      ...
      st.playInRound1 = [
        { id: 'K-P1-M1', team1: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), team2: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), winner: null, bestOf: 3, isGrandFinal: false },
        { id: 'K-P1-M2', team1: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), team2: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), winner: null, bestOf: 3, isGrandFinal: false },
        { id: 'K-P1-M3', team1: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), team2: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), winner: null, bestOf: 3, isGrandFinal: false },
        { id: 'K-P1-M4', team1: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), team2: (() => { let team = getRandomTeam(initialUsedTeams, region); initialUsedTeams.push(team); return team.name; })(), winner: null, bestOf: 3, isGrandFinal: false },
      ];

      // Initialize upper bracket Round 1 (where bye teams join)
      st.ubRound1 = [
          { id: 'K-UB1-M1', team1: byeTeams[0], team2: st.series['K-P1-M1']?.winner || '', winner: null, bestOf: 3, isGrandFinal: false }, 
          { id: 'K-UB1-M2', team1: byeTeams[1], team2: st.series['K-P1-M2']?.winner || '', winner: null, bestOf: 3, isGrandFinal: false }, 
          { id: 'K-UB1-M3', team1: byeTeams[2], team2: st.series['K-P1-M3']?.winner || '', winner: null, bestOf: 3, isGrandFinal: false }, 
          { id: 'K-UB1-M4', team1: byeTeams[3], team2: st.series['K-P1-M4']?.winner || '', winner: null, bestOf: 3, isGrandFinal: false }, 
      ];
      ...
      st.region = region; // Ensure region is saved in the state
      st.dirty = false;
      saveKickoffState(st, region, activeSaveId);
    }
...
function kickoffWatchSeries(id, team1, team2, bestOf, isGrandFinal) {
  ...
  while (t1MapWins < mapsToWin && t2MapWins < mapsToWin) {
    ...
    const matchSim = new MatchSimulator(team1Obj, team2Obj, [], strategies, mapName);
  ...
  const activeSaveId = localStorage.getItem('activeSaveId');
  const playerTeamData = teams.find(t => String(t.id) === String(activeSave?.teamId));
  const currentRegion = playerTeamData ? playerTeamData.region : "Americas";
  const st = getKickoffState(currentRegion, activeSaveId);
  const region = st.region || currentRegion;
  ...
  saveKickoffState(st, currentRegion, activeSaveId);
  
  localStorage.setItem('rerenderKickoff', Date.now());
  window.parent.postMessage({ type: 'rerenderKickoff', region: region }, window.location.origin);
  renderKickoff(st); 
}
...
function handleChampionshipPoints(matchId, winner, loser) {
    ...
    // Official VCT points for Kickoff:
    // 1 point per match win.
    // 3 points for the tournament winner.
    
    // 1 point for THIS match win
    activeSave.championshipPoints[winner] = (activeSave.championshipPoints[winner] || 0) + 1;
    console.log(`CP: ${winner} gets +1 point for a match win in Kickoff.`);

    if (matchId === 'K-GF') {
        // Bonus 3 points for winning Kickoff
