...
export function automateMastersTournament(qualifiedTeamsRaw, savePlayers, simulateMatchFn, currentState = null, playerTeamName = null) {
    ...
    if (activeTeams.length > 0) {
        ...
        Object.keys(teamsByScore).forEach(score => {
            ...
            if (currentSwissRound === 1) {
                // Simple greedy approach: Shuffle and try to pair different regions
    ...
    if (state.playoffs.semifinals.length === 2 && !state.playoffs.grandFinal) {
        ...
        // 3rd Place Match
        if (l1 && l2) {
            const tpMatchId = 'M-PLAYOFF-3RD';
            console.log(`Generating 3rd Place matchup for ${l1} vs ${l2}`);
            state.playoffs.matches[tpMatchId] = {
                team1: l1, team2: l2,
                winner: null,
                loser: null,
                score: null,
            ...
            state.playoffs.thirdPlace = tpMatchId;
        }
    ...
    if (state.playoffs.grandFinal) {
        ...
        // Ensure 3rd place match is also finished if it exists
        const tpMatch = state.playoffs.thirdPlace ? state.playoffs.matches[state.playoffs.thirdPlace] : null;
        const tpFinished = !tpMatch || (tpMatch && tpMatch.winner);

        if (gfMatch && gfMatch.winner && tpFinished) {
            state.complete = true;
        }
    }
